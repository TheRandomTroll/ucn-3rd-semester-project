...
{
    public class CommentsControllerCreateTests
    {
        [Fact]
        public async void Create_ReturnsCreated_WhenEverythingIsOkayWithInput()
        {
            // Arrange 
            ...
            var controller = new CommentsController(usersRepository, commentsRepository, reportsRepository,
                mapper)
            {
                ControllerContext =
                {
            ...
            // Act
            var result = await controller.CreateAsync(new CreateCommentDto {ReportId = report.Id.ToString()});
